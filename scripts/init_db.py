...
"""
scripts/init_db.py
Initialise la base de données via SQLAlchemy ORM (models.py).
"""
...
async def init_database(drop_all: bool = False) -> None:
    ...
    import src.database.models  # noqa — enregistre tous les modèles

    logger.info("Connexion à : {}", settings.database.url.split("@")[-1])

    engine = create_async_engine(settings.database.url, echo=False)

    async with engine.begin() as conn:

        # Extensions PostGIS (optionnelles)
        for ext in ["postgis", "pg_trgm", "uuid-ossp"]:
            try:
                ...
                logger.warning("⚠️  Extension {} ignorée : {}", ext, e)
        ...
        logger.info("✅ Tables créées depuis models.py")
        ...
        logger.info("Tables dans la base ({}) :", len(tables))
        for t in tables:
            logger.info("   • {}", t)
...
if __name__ == "__main__":
    ...
    asyncio.run(init_database(drop_all=drop_all))